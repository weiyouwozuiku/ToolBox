...
def readJson():
    try:
        #用户配置信息
        with open(os.getcwd()+os.sep+version+os.sep+'config.json','r') as fp:
            users = json.load(fp)
            return users
    except Exception as e:
        print(traceback.format_exc())
...
def main(event, context):
    ...
    for user in users:
        #清空上一个用户的日志记录
        open(os.getcwd()+os.sep+version+os.sep+'log.txt',mode='w',encoding='utf-8')
        global client
        ...
        if ('email' in user) :
            notify.sendEmail(user['sendEmail'],user['receiveEmail'],user['smtpPassword'],user['smtpServer'])
